...
const resourceColumns = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Genealogy Search', href: '/genealogists/search' },
    ],
  },
];
    ...
    <>
      {links.map((link, index) =>
        link.href ? (
          ...
          <a key={index} href="#" className="block font-mono text-xs text-neutral-700 hover:text-neutral-900 py-1">
            {link.label}
          </a>
        )
      )}
...
export function MobileMenu({ isOpen, onClose, previewContent }: MobileMenuProps) {
  ...
  const sections = [
    ...
    { key: 'resources', label: 'Resources', content: null },
  ...
  return (
    <div className="md:hidden border-t border-neutral-300 bg-white max-h-[calc(100vh-80px)] overflow-y-auto">
      <nav className="container mx-auto px-4 py-6 space-y-4">
        {sections.map((section, idx) => (
          <div key={section.key} className={idx < sections.length - 1 ? 'border-b border-neutral-200 pb-4' : 'pb-4'}>
            ...
            {section.key === 'resources' && expandedSection === 'resources' && (
              <div className="pl-4 space-y-4 mt-3">
                {resourceColumns.map((column, index) => (
                  <div key={index} className="space-y-2">
                    <h4 className="font-mono text-xs text-neutral-800 border-b border-neutral-300 pb-1">{column.heading}</h4>
                    {column.links.map((link, linkIndex) =>
                      link.href ? (
                        ...
                        <a key={linkIndex} href="#" className="block font-mono text-xs text-neutral-700 hover:text-neutral-900 py-1">
                          {link.label}
                        </a>
                      )
                    )}
            ...
            {section.key !== 'resources' && expandedSection === section.key && section.content && (
              <div className="pl-4 space-y-2 mt-3">
                <p className="font-mono text-xs text-neutral-600 mb-3">{section.content.description}</p>
