...
const emptyData: WizardData = {
  isOrganization: null, goal: '', rwdiInterest: false,
  pathway: '',
  ...
  emailCopyRequested: false, receiptEmail: '',
};
...
function FieldLabel({ htmlFor, required, children }: { htmlFor?: string; required?: boolean; children: React.ReactNode }) {
  return (
    <label htmlFor={htmlFor} className="block text-sm font-medium text-neutral-800 mb-1.5">
      {children}
      {required && <span className="text-neutral-500 ml-1" aria-hidden="true">*</span>}
    </label>
  );
}
...
function TextInput({ id, value, onChange, placeholder, type = 'text', required }: {
  ...
  return (
    <input
      ...
      className="w-full border border-neutral-300 px-3 py-2.5 text-sm text-neutral-800 bg-white placeholder:text-neutral-400 focus:outline-2 focus:outline-offset-0 focus:outline-neutral-800 hover:border-neutral-400 transition-colors"
    />
  );
}
...
function SelectInput({ id, value, onChange, options, placeholder, required }: {
  ...
  return (
    <select
      ...
      required={required}
      aria-required={required}
      className="w-full border border-neutral-300 px-3 py-2.5 text-sm text-neutral-800 bg-white focus:outline-2 focus:outline-offset-0 focus:outline-neutral-800 hover:border-neutral-400 transition-colors appearance-none"
    >
      ...
      {options.map(opt => (
        <label
          ...
          className={`flex items-start gap-3 p-3 border cursor-pointer transition-colors ${
            value === opt.value ? 'border-neutral-800 bg-neutral-50' : 'border-neutral-200 hover:border-neutral-400'
          }`}
        >
          <input
            type="radio"
          ...
          <div>
            ...
            {opt.hint && <p className="text-xs text-neutral-500 mt-0.5">{opt.hint}</p>}
          </div>
        </label>
      ))}
    </fieldset>
...
function PathwayCard({ id, title, desc, bullets, selected, onSelect }: {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`w-full text-left border p-5 transition-all focus:outline-2 focus:outline-offset-2 focus:outline-neutral-900 ${
        selected ? 'border-neutral-900 bg-neutral-50 shadow-sm' : 'border-neutral-200 hover:border-neutral-400'
      }`}
...
function Step0({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  return (
    <div className="space-y-8">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Is your organization eligible?</h2>
        <p className="text-sm text-neutral-500">Recollection Wisconsin partners with institutions, not individuals.</p>
      </div>
      ...
      {data.isOrganization === false && (
        <div className="border border-neutral-300 bg-neutral-50 p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-neutral-700">
            <p className="font-medium mb-1">RW partners with institutions, not individuals.</p>
            <p>If you have materials of historical significance, consider reaching out to your county historical society, public library, or local museum — they may be able to help digitize and share your collection through their partnership with RW.</p>
          </div>
        </div>
      )}

      <div>
        <FieldLabel htmlFor="goal">What best describes your goal? <span className="text-neutral-400 font-normal">(optional)</span></FieldLabel>
        <RadioGroup
          name="goal"
          value={data.goal}
          ...
          options={[
            ...
            { value: 'starting-digitization', label: 'I\'m just getting started digitizing materials.', hint: 'You are in early planning and want guidance on next steps.' },
            { value: 'digitize', label: 'I need help digitizing materials.', hint: 'You have physical items and need scanning or metadata support.' },
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        <input
          type="checkbox"
          checked={data.rwdiInterest}
          onChange={e => setData({ ...data, rwdiInterest: e.target.checked })}
          className="mt-0.5 accent-neutral-800"
        />
        <div>
          <span className="text-sm text-neutral-800">I'd like to learn more about hosting a student intern to assist with digitization, description, and uploading.</span>
          <p className="text-xs text-neutral-500 mt-0.5">Placement is limited. A colleague who coordinates this program will follow up.</p>
        </div>
      </label>
    </div>
  );
...
function Step1({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Choose your pathway</h2>
        <p className="text-sm text-neutral-500">Select the option that best fits your organization's situation. You can always ask for help deciding.</p>
      </div>

      <div className="space-y-3">
        <PathwayCard
          id="hosting"
          title="Hosting (CONTENTdm)"
          desc="RW hosts your collection in partnership with Milwaukee Public Library."
          bullets={[
            'Best when you do not have your own digital repository',
            'RW manages technical infrastructure and upload instructions',
            'You can begin planning before digitization is complete',
          ]}
        ...
        <PathwayCard
          id="harvesting"
          title="Harvesting (OAI-PMH)"
          desc="RW harvests metadata from your existing platform via OAI-PMH."
          bullets={['You already have a digital repository', 'Your platform supports OAI-PMH', 'Harvesting occurs every three months']}
      ...
      <p className="text-xs text-neutral-500">
        Not sure?{' '}
        <Link to="/organizations/contributor-faq" className="underline hover:text-neutral-800" target="_blank">Read the FAQ</Link>
...
function Step2({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">About your organization</h2>
        <p className="text-sm text-neutral-500">Tell us about your institution and who we'll be working with.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="md:col-span-2">
          <FieldLabel htmlFor="website">Website URL <span className="text-neutral-400 font-normal">(optional)</span></FieldLabel>
...
function Step3({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">About your collection</h2>
        <p className="text-sm text-neutral-500">Give us a snapshot of what you'd like to share.</p>
      </div>
      ...
      <div>
        <FieldLabel required>Material formats <span className="text-neutral-400 font-normal">(select all that apply)</span></FieldLabel>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {FORMAT_OPTIONS.map(fmt => (
            <label key={fmt} className={`flex items-center gap-2 p-2.5 border cursor-pointer text-sm transition-colors ${
              data.formats.includes(fmt) ? 'border-neutral-800 bg-neutral-50' : 'border-neutral-200 hover:border-neutral-400'
            }`}>
              <input
                type="checkbox"
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        ...
        />
        <div>
          <span className="text-sm text-neutral-800">This collection documents underrepresented or underserved Wisconsin communities</span>
          <p className="text-xs text-neutral-500 mt-0.5">RW prioritizes these collections; we can discuss more how the digital materials and metadata are handled.</p>
        </div>
      </label>
    </div>
  );
...
function Step4({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Rights & ownership</h2>
        <p className="text-sm text-neutral-500">Understand how ownership works before you proceed.</p>
      </div>
      ...
      <div className="border border-neutral-200 bg-neutral-50 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-800 mb-1">You retain ownership of your materials.</p>
      ...
      <div>
        <p className="text-sm font-medium text-neutral-800 mb-3">CC0 Metadata Acknowledgement <span className="text-neutral-500">*</span></p>
        <label className={`flex items-start gap-3 p-4 border cursor-pointer transition-colors ${
          data.cc0Acknowledged ? 'border-neutral-800 bg-neutral-50' : 'border-neutral-300 hover:border-neutral-500'
        }`}>
          <input
            type="checkbox"
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-block text-sm text-neutral-700 hover:underline mt-2"
        >
          What is CC0?
        </a>
      </div>
...
function Step5({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Digital readiness</h2>
        <p className="text-sm text-neutral-500">Tell us about your current technical setup. "Unsure" is always a valid answer.</p>
      </div>
      ...
      {data.hasPlatform === 'yes' && (
        <div>
          <FieldLabel htmlFor="platformName">Which platform? <span className="text-neutral-400 font-normal">(optional)</span></FieldLabel>
          ...
          {data.pathway === 'harvesting' && data.hasOaiPmh === 'no' && (
            <div className="mt-3 border border-neutral-300 bg-neutral-50 p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-neutral-700">
                OAI-PMH is required for Harvesting. We can help assess your platform's capabilities — or discuss whether <Link to="/organizations/hosting" className="underline hover:text-neutral-900" target="_blank">Hosting</Link> may be a better fit. Continue with your application and we'll work through this together.
              </p>
            </div>
          )}
      ...
      <div>
        ...
        {data.needsHelp && (
          <p className="text-xs text-neutral-500 mt-2">
            Check out our <Link to="/organizations/resources" className="underline hover:text-neutral-800" target="_blank">Resources library</Link> for guides. We'll also discuss this during your discovery call.
          </p>
        )}
      </div>
    </div>
...
function Step6({ data, setData }: { data: WizardData; setData: (d: WizardData) => void }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Timeline & support preferences</h2>
        <p className="text-sm text-neutral-500">Help us understand your timeline and how best to follow up.</p>
      </div>
...
function Step7({
  ...
  const pathwayLabel = { hosting: 'Hosting (CONTENTdm)', harvesting: 'Harvesting (OAI-PMH)', '': 'Not selected' };
  const copyEmail = (data.contactEmail || data.receiptEmail).trim();
  const needsReceiptEmail = data.emailCopyRequested && !copyEmail;
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-neutral-900 mb-1">Review your submission</h2>
        <p className="text-sm text-neutral-500">Check your responses before submitting. Click "Edit" to go back to any section.</p>
      </div>
      ...
      <div className="border border-neutral-200 p-4">
        ...
        <ReviewRow label="Intern support interest" value={data.rwdiInterest ? 'Yes' : 'No'} onEdit={() => goToStep(0)} />
      </div>

      {/* Org */}
      <div className="border border-neutral-200 p-4">
        <p className="text-xs font-mono uppercase tracking-widest text-neutral-400 mb-3">Your Organization</p>
      ...
      {/* Collection */}
      <div className="border border-neutral-200 p-4">
        <p className="text-xs font-mono uppercase tracking-widest text-neutral-400 mb-3">Your Collection</p>
      ...
      <div className="border border-neutral-200 p-4">
        <p className="text-xs font-mono uppercase tracking-widest text-neutral-400 mb-3">Submission receipt</p>
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
            checked={data.emailCopyRequested}
            onChange={e => setData({ ...data, emailCopyRequested: e.target.checked })}
            className="mt-0.5 accent-neutral-800"
          />
          <span className="text-sm text-neutral-800">Email me a copy of my completed submission.</span>
        </label>

        {data.emailCopyRequested && !data.contactEmail && (
          <div className="mt-3">
            <FieldLabel htmlFor="receiptEmail" required>Receipt email</FieldLabel>
            <TextInput
              id="receiptEmail"
              type="email"
              value={data.receiptEmail}
              onChange={v => setData({ ...data, receiptEmail: v })}
        ...
        {data.emailCopyRequested && copyEmail && (
          <p className="text-xs text-neutral-500 mt-3">
            A copy will be sent to: {copyEmail}
          </p>
        )}
      </div>
      ...
      <button
        type="button"
        onClick={onSubmit}
        disabled={!data.cc0Acknowledged || submitting || needsReceiptEmail}
...
export function GetStarted() {
  ...
  const initialPathway: Pathway = rawPathway === 'hosting' || rawPathway === 'harvesting' ? rawPathway : '';
  ...
  useEffect(() => {
    setData(d => ({ ...d, pathway: initialPathway }));
  }, [initialPathway]);
  ...
  const handleSubmit = () => {
    setSubmitting(true);
    const receiptEmail = data.contactEmail || data.receiptEmail;
    setTimeout(() => {
      navigate('/organizations/confirmation', {
        state: {
          pathway: data.pathway,
          orgName: data.orgName,
          contactEmail: receiptEmail,
          emailCopyRequested: data.emailCopyRequested,
          rwdiInterest: data.rwdiInterest,
        },
      });
    }, 1200);
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white">
        {/* Progress bar */}
        <div className="border-b border-neutral-200 bg-neutral-50 sticky top-16 z-20">
          <div className="container mx-auto px-4 md:px-6 py-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-neutral-500 font-mono">Step {step + 1} of {STEPS.length}: <strong className="text-neutral-700">{STEPS[step]}</strong></span>
              <span className="text-xs text-neutral-400 font-mono">{progressPct}% complete</span>
            </div>
            <div className="h-1 bg-neutral-200 w-full">
              <div
                className="h-1 bg-neutral-800 transition-all duration-300"
              ...
              <ol className="flex" aria-label="Wizard steps">
                {STEPS.map((s, i) => (
                  <li key={s} className="flex-1 text-center">
                    <div className={`py-2 text-xs border-b-2 transition-colors ${
                      i === step ? 'border-neutral-900 text-neutral-900 font-medium' :
                      i < step ? 'border-neutral-400 text-neutral-500' :
        ...
        <div className="container mx-auto px-4 md:px-6 py-10 md:py-14">
          <div className="max-w-2xl mx-auto">
            ...
            {step === 7 && <Step7 data={data} setData={setData} goToStep={setStep} onSubmit={handleSubmit} submitting={submitting} />}
            ...
            {step < 7 && (
              <div className="flex items-center justify-between mt-10 pt-6 border-t border-neutral-200">
                <button
                  ...
                  disabled={step === 0}
                  className="border border-neutral-300 px-6 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed focus:outline-2 focus:outline-offset-2 focus:outline-neutral-900"
                >
                  ← Back
                </button>
                ...
                <button
                  ...
                  disabled={!canAdvance()}
                  className="inline-flex items-center gap-2 bg-neutral-900 text-white px-7 py-2.5 text-sm hover:bg-neutral-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed focus:outline-2 focus:outline-offset-2 focus:outline-neutral-900"
                >
            ...
            {step === 0 && data.isOrganization === false && (
              <div className="mt-6 text-center">
                <p className="text-sm text-neutral-500 mb-3">Want to learn more about Recollection Wisconsin?</p>
                <Link to="/" className="text-sm text-neutral-700 hover:underline">← Back to homepage</Link>
              </div>
            )}
          </div>
        </div>
